    ...
    public JeuReponse create(JeuReponse obj){
        ...
        try {
         this.connect.createStatement(
                    ...
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("INSERT INTO `jeureponses` (`LIBELLE`, `IDJEUREP`, `REP1`, `REP2`) VALUES ('"+libelle+"', '"+idjeurep+"', '"+rep1+"', '"+rep2+"');");
...
// Méthode qui supprime le jeu de réponse grâce a son idjeurep et supprime toutes les questions associées a cet idjeurep avec la méthode QuestionDAO.delete()
    ...
    public JeuReponse delete(JeuReponse obj){
        ...
        try {
            ...
            this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("DELETE FROM `jeureponses` WHERE `jeureponses`.`LIBELLE` = '"+libelle+"' AND `jeureponses`.`IDJEUREP` = "+idjeurep+"");
...
JeuReponse obj = new JeuReponse();
        ...
        try {
            ...
            this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("DELETE FROM `jeureponses` WHERE `jeureponses`.`LIBELLE` = '"+libelle+"'");
