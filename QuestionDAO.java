...
System.out.println("INSERT INTO `questions` (`LIBELLE`, `IDJEUREP`, `IDQUEST`, `INTITULE`, `BONNEREP`) VALUES ( "+libelle+", "+idjeurep+","+idQuest+", "+intitule+", "+bonnerep+");" );

        try {
          this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("INSERT INTO `questions` (`LIBELLE`, `IDJEUREP`, `IDQUEST`, `INTITULE`, `BONNEREP`) VALUES ( '"+libelle+"', '"+idjeurep+"','"+idQuest+"', '"+intitule+"', '"+bonnerep+"');" );
...
// Methode de suppression d'une question en passant un objet de type Question en param
    public Question delete(Question obj) {
        ...
        // libelle = theme des questions , inititule = questions
        System.out.println("DELETE FROM `questions` WHERE `questions`.`LIBELLE` = '"+libelle+"' AND `questions`.`IDJEUREP` = "+idjeurep+" AND `questions`.`IDQUEST` = "+idQuest+"" );

        try {
            this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("DELETE FROM `questions` WHERE `questions`.`LIBELLE` = '"+libelle+"' AND `questions`.`IDJEUREP` = "+idjeurep+" AND `questions`.`IDQUEST` = "+idQuest+"" );
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
// Methode d'update d'une question en passant en param un obj de type Question
    public Question update(Question obj) {
        ...
        // libelle = theme des questions , inititule = questions
        System.out.println("UPDATE `questions` SET `INTITULE` = '"+intitule+"', `BONNEREP` = '"+bonnerep+"' WHERE `questions`.`LIBELLE` = '"+libelle+"' AND `questions`.`IDJEUREP` = "+idjeurep+" AND `questions`.`IDQUEST` = "+idQuest+";" );

        try {
            this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeUpdate("UPDATE `questions` SET `INTITULE` = '" + intitule + "', `BONNEREP` = '" + bonnerep + "' WHERE `questions`.`LIBELLE` = '" + libelle + "' AND `questions`.`IDJEUREP` = " + idjeurep + " AND `questions`.`IDQUEST` = " + idQuest + ";");
        } catch (SQLException e) {
            e.printStackTrace();
        }
